...
unmount_device() {
	...
	if [ $? -eq 0 ]; then
		echo -ne "$DEVICE listed in /proc/mounts, attempting to unmount...\n"
...
if [ -e "$GRUB_CFG" ]; then
	...
	sed -i "s/timeout=10/timeout=5/g" $GRUB_CFG

	sed -i "s@ root=[^ ]*@ @" $GRUB_CFG
	sed -i "s@vmlinuz @vmlinuz root=$TARGET_ROOTFS rw rootwait @" $GRUB_CFG
fi
...
if [ -d "$GUMMI_ENTRIES" ]; then
	...
	sed -i "s@options *LABEL=boot @options LABEL=Boot root=$TARGET_ROOTFS rw rootwait @" $GUMMI_CFG
fi
...
printf '%s' "fs0:\EFI\BOOT\bootx64.efi" > $BOOTFS_MNT/startup.nsh
